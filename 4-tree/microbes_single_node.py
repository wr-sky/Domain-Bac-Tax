...
class Microbes(object):
    ...
    def compute_distance(self, defined_microbes, method, process_num, p_rank, output, history_dir, history_dir_2):
        microbes = pd.read_csv(defined_microbes, sep='\t', index_col=False, dtype={"GCF": object})
        microbes_history = pd.read_csv(history_dir, sep='\t', index_col=False, dtype={"GCF": object})
        microbes_history_2 = pd.read_csv(history_dir_2, sep='\t', index_col=False, dtype={"GCF": object})
        distances = dict()
        for i in range(microbes.shape[0]):
            if i % process_num == p_rank:
                ...
                for j in range(microbes_history.shape[0]):
                    gcfB = microbes_history.iloc[j]['GCF']
                    sizeB = microbes_history.iloc[j]['Size']
                    microbeB = list(microbes_history.iloc[j]['Modules'].split(','))
                    assert sizeB == len(microbeB), "[B]microbe {} genome size conflict.".format(gcfB)
                    ...
                    if method == 'jaccard':
                        ...
                        jd = round((smaller - sizeOfAandB)/smaller, 6)
                    distances['-'.join((gcfA, gcfB))] = jd
                for j in range(microbes_history_2.shape[0]):
                    gcfB = microbes_history_2.iloc[j]['GCF']
                    sizeB = microbes_history_2.iloc[j]['Size']
                    microbeB = list(microbes_history_2.iloc[j]['Modules'].split(','))
                    assert sizeB == len(microbeB), "[B]microbe {} genome size conflict.".format(gcfB)
    ...
    def compute_jaccard_distances(self, by, defined_microbes, method, output, history_dir, history_dir_2, redefine_microbes=False, sep='-'):
        ...
        if by in ['vector_freq', 'module_freq']:
            ...
            for p_rank in range(process_num):
                proc = mp.Process(target=self.compute_distance, args=(defined_microbes, method, process_num, p_rank, output, history_dir, history_dir_2))
            ...
            for i in range(microbes.shape[0]):
                ...
                for j in range(microbes_history.shape[0]):
                    gcfB = microbes_history.iloc[j]['GCF']
                    sizeB = microbes_history.iloc[j]['Size']
                    microbeB = set(microbes_history.iloc[j]['Modules'].split(','))
                    assert sizeB == len(microbeB), "[B]microbe {} genome size conflict.".format(gcfB)
                    sizeOfAandB = len(microbeA.intersection(microbeB))
                    sizeOfAorB = sizeA + sizeB - sizeOfAandB
                    if method == 'jaccard':
                ...
                for j in range(microbes_history_2.shape[0]):
                    gcfB = microbes_history_2.iloc[j]['GCF']
                    sizeB = microbes_history_2.iloc[j]['Size']
                    microbeB = set(microbes_history_2.iloc[j]['Modules'].split(','))
                    assert sizeB == len(microbeB), "[B]microbe {} genome size conflict.".format(gcfB)
                    sizeOfAandB = len(microbeA.intersection(microbeB))
                    sizeOfAorB = sizeA + sizeB - sizeOfAandB
                    if method == 'jaccard':
...
def main():
    pfam_scan_out_dir = "/home/wbq/1/Bacteria/single_node/Spirochaetes_order/pfam_scan_out"
    output_dir = "/home/wbq/1/Bacteria/single_node/Spirochaetes_order/network_out"

    microbes = Microbes(pfam_scan_out_dir=pfam_scan_out_dir,
    ...
    for define_by in ['vector_freq', 'vector', 'module', 'module_freq']:
        ...
        defined_microbes_history = "/home/wbq/1/network_out/{}_defined_microbes.tsv".format(define_by)
        defined_microbes_history_2 = "/home/wbq/1/Bacteria/single_node/network_out/{}_defined_microbes.tsv".format(define_by)
        if not os.path.exists(defined_microbes):
            microbes.define_microbes(by=define_by, output=defined_microbes)
        for define_method in ['jaccard', 'do']:
            jaccard_distances = output_dir + "/{}/{}_genomes_{}_distances.json".format(define_method, define_by, define_method)
            microbes.compute_jaccard_distances(by=define_by, defined_microbes=defined_microbes, method=define_method, output=jaccard_distances, history_dir=defined_microbes_history, history_dir_2=defined_microbes_history_2, redefine_microbes=False)
