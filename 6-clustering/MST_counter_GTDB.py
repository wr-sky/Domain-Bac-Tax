...
def find_mst():
    phylum_dict = dict()
    ref = "D:\\1-研究课题\\1-基于功能模块迁移的细菌重组网络研究\\4-论文-分类\\7-JB\\bac120_taxonomy_r207.tsv.tsv\\bac120_taxonomy_r207.tsv"
    with open(ref, "r") as rin:
        for line in rin.readlines():
            # 对于输入文件1，可以自行选择一列作为关键字对MST图进行分类
            gcf = line.split('\t')[0].split('_')[-1].split('.')[0]
            phylum = line.split('\t')[1].split(';')[1].split('__')[1].split('_')[0]
            ...
            for key in key_set:
                if key not in used:
                    ...
                    while len(tmp) != 0:
                        tmp_2 = list()
                        for tmp_iter in tmp:
                            if tmp_iter not in phylum_dict.keys() or phylum_dict[tmp_iter] == phylum_ref:
                                count += 1
